...
def parse_bb_variables(cfg_str):
    global var_map, array_dim_info, temp_reg_index_block
        ...
        for line in code_text.split('\n'):
            ...
            if var_match:
                ...
                if global_var_match:
                    ...
                    # 如果是数组变量，直接添加提取的部分
                    base_type, reg_name, var_name = pattern_tool(line, '* @')
                    if not base_type:
                       continue
                    reg_name = reg_name[reg_name.find(array_var_name):]
                    ...
                    if var_name:
                        temp_reg_index = re.search(r'(%\d+)', reg_name)
                        if temp_reg_index:
                            reg_index = temp_reg_index.group(1).strip()
                            if var.startswith('store ') and int(split_tool(reg_index, '%')) <= max_temp:
                                temp_reg_index_block[func_name][black_name].append(reg_index)
                    ...
                    if reg_name in var_map:
                        ...
                        while queue:
                            ...
                            if global_var_match:
                                array_var_name = global_var_match[-1]
                                base_type, reg_name, var = pattern_tool(current_value, '* @')
                                if not base_type:
                                    continue
                                reg_offset = reg_name.strip().split(',')
                                curr_offset = register_offset(reg_offset)
...
def main():
    global array_dim_info, array_loop_info, array_access_info, seq_number, temp_reg_index_block
    ...
    try:
        ...
        export_to_file(temp_reg_index_block, f'{file_path}global_reg_index_block.json')
